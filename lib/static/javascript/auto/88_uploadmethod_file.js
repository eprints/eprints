var Screen_EPrint_UploadMethod_File = Class.create({
    ...
    new EPrints.DragAndDrop.Files (this, this.container, $(this.prefix + '_file'));
	...
	createFile: function(file) {
		...
		new Ajax.Request(url + '?ajax=1', {
			...
			onSuccess: (function(transport) {
				...
				file.docid = json['document']['docid'];
				file.fileid = json['file']['fileid'];
				button.update (json['phrases']['abort']);
				button.show();
				this.postFile (file, json['file']['filesize']);
			}).bind (this),
			parameters: params
		});
	},
