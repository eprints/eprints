...
Lightbox.prototype = {
	...
	var scrn = this.getViewportSize();
	scrn[0] -= LightboxOptions.borderSize * 2;
	scrn[1] -= LightboxOptions.borderSize * 2;
	var ratio = imgWidth / imgHeight;
	if (imgWidth > scrn[0]) {
		imgWidth = scrn[0];
		imgHeight = Math.round(scrn[0] / ratio);
	}
	if (imgHeight > scrn[1]) {
		imgHeight = scrn[1];
		imgWidth = Math.round(scrn[1] * ratio);
	}
