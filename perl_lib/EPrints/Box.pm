...
sub EPrints::Box::render
{
	...
	if( !defined $options{handle} ) { EPrints::abort( "EPrints::Box::render called without a session. Bad bad bad." ); }
	...
	my $div = $handle->make_element( "div", class=>"ep_summary_box", id=>$id );

	# Title
	my $div_title = $handle->make_element( "div", class=>"ep_summary_box_title" );
	$div->appendChild( $div_title );

	my $nojstitle = $handle->make_element( "div", class=>"ep_no_js" );
	$nojstitle->appendChild( $handle->clone_for_me( $options{title},1 ) );
	$div_title->appendChild( $nojstitle );

	my $collapse_bar = $handle->make_element( "div", class=>"ep_only_js", id=>$colbarid );
	my $collapse_link = $handle->make_element( "a", class=>"ep_box_collapse_link", onclick => "EPJS_blur(event); EPJS_toggleSlideScroll('${contentid}',true,'${id}');EPJS_toggle('${colbarid}',true);EPJS_toggle('${barid}',false);return false", href=>"#" );
	$collapse_link->appendChild( $handle->make_element( "img", alt=>"-", src=>$options{hide_icon_url}, border=>0 ) );
	$collapse_link->appendChild( $handle->make_text( " " ) );
	$collapse_link->appendChild( $handle->clone_for_me( $options{title},1 ) );
	...
	my $uncollapse_bar = $handle->make_element( "div", class=>"ep_only_js", id=>$barid );
	my $uncollapse_link = $handle->make_element( "a", id=>$barid, class=>"ep_box_collapse_link", onclick => "EPJS_blur(event); EPJS_toggleSlideScroll('${contentid}',false,'${id}');EPJS_toggle('${colbarid}',$a);EPJS_toggle('${barid}',$b);return false", href=>"#" );
	$uncollapse_link->appendChild( $handle->make_element( "img", alt=>"+", src=>$options{show_icon_url}, border=>0 ) );
	$uncollapse_link->appendChild( $handle->make_text( " " ) );
	$uncollapse_link->appendChild( $handle->clone_for_me( $options{title},1 ) );
	$uncollapse_bar->appendChild( $uncollapse_link );
	$div_title->appendChild( $uncollapse_bar );
	
	# Body	
	my $div_body = $handle->make_element( "div", class=>"ep_summary_box_body", id=>$contentid );
	my $div_body_inner = $handle->make_element( "div", id=>$contentid."_inner", style=>$options{content_style} );
