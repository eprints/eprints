######################################################################
#
# EPrints::Plugin::Sword::Import::Png
#
######################################################################
#
#  __COPYRIGHT__
...
package EPrints::Plugin::Sword::Import::Png;
...
sub new
{
	...
	$self->{name} = "SWORD Importer - PNG";
	$self->{visible} = "all";
	
	$self->{accept} = "image/x-png";
...
sub input_file
{
	...
	# We need to get ALL the files of type 'image/png'
	my $f = EPrints::Sword::Utils::get_file_to_import( $session, $opts{files}, "image/x-png", 1 );
	...
	if(!defined $dataset)
        {
                print STDERR "\n[SWORD-PNG] [INTERNAL-ERROR] Failed to open the dataset '$dataset_id'.";
                return;
        }
	...
	foreach my $infile (@files)
	{
		...
		my $document = EPrints::DataObj::Document->create_from_data( $session, \%doc_data, $doc_dataset );
                ...
                if(!defined $document)
                {
                        print STDERR "\n[SWORD-PNG] [ERROR] Failed to add the attached file to the eprint.";
	        }
		else
		{
