...
sub xml_dataobj
{
	...
	my $didl = $plugin->{handle}->make_element( 
		"didl:DIDL",
	...
	my $item = $plugin->{handle}->make_element( "didl:Item" );
	$didl->appendChild( $item );


	my $d1 = $plugin->{handle}->make_element( "didl:Descriptior" );
	my $s1 = $plugin->{handle}->make_element( "didl:Statement", mimeType=>"application/xml; charset=utf-8" );
	my $ident = $plugin->{handle}->make_element( 
		"dii:Identifier",
	...
	$ident->appendChild( $plugin->{handle}->make_text( $eprint->get_url ) );
	$s1->appendChild( $ident );
	$d1->appendChild( $s1 );
	$item->appendChild( $d1 );


	my $d2 = $plugin->{handle}->make_element( "didl:Descriptior" );
	my $s2 = $plugin->{handle}->make_element( "didl:Statement", mimeType=>"application/xml; charset=utf-8" );
	my $dc_plugin = $plugin->{handle}->plugin( "Export::OAI_DC" );
	...
	{
		my $comp = $plugin->{handle}->make_element( "didl:Component" );
		$item->appendChild( $comp );


		my $d3 = $plugin->{handle}->make_element( "didl:Descriptior" );
		my $s3 = $plugin->{handle}->make_element( "didl:Statement", mimeType=>"application/xml; charset=utf-8" );
		my $i3 = $plugin->{handle}->make_element( 
			"dii:Identifier",
			"xmlns:dii"=>"urn:mpeg:mpeg21:2002:01-DII-NS",
			"xmlns:xsi"=>"http://www.w3.org/2001/XMLSchema-instance",
			"xsi:schemaLocation"=>"urn:mpeg:mpeg21:2002:01-DII-NS
		 	    http://standards.iso.org/ittf/PubliclyAvailableStandards/MPEG-21_schema_files/dii/dii.xsd" );
		$i3->appendChild( $plugin->{handle}->make_text( $doc->get_baseurl ) );
		$s3->appendChild( $i3 );
		$d3->appendChild( $s3 );
		$comp->appendChild( $d3 );
		...
		{
			my $res = $plugin->{handle}->make_element( "didl:Resource", 
	#				mimeType=>$format,
