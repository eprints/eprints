...
TODO: {
	local $TODO = "API still under development";
	...
	my $username = $eprint->value( "username" );
	...
	my $email = $eprint->value( "email" );
	my $user3 = $repo->user_by_email( $username );
	isa_ok( $user3, "EPrints::DataObj::User", "Get a user object by email from the repository" );
